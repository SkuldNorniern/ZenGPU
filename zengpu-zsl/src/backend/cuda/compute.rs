...
use crate::backend::cuda::{ENTRY, CudaShader};
...
fn emit_stmt(s: &mut String, ctx: &Ctx<'_>, stmt: &IrStmt, depth: usize) {
    match stmt {
        IrStmt::Let { name, init } => {
            ...
            indent(s, depth);
            let _ = writeln!(s, "{} {} = {};", cuda_scalar(ty), name, emit_expr(init));
        }
...
fn emit_expr(e: &IrExpr) -> String {
    match e {
        ...
        IrExpr::VecConstruct { dim, args } => {
            ...
            format!("make_float{}({})", dim, args_s.join(", "))
        }
        IrExpr::Extend { base, scalar } => {
            format!("make_float4({}, {})", emit_expr(base), emit_expr(scalar))
        }
        ...
        IrExpr::Builtin { func, args } => {
            ...
            match func {
                ...
                BuiltinFn::Normalize => {
                    // CUDA has no built-in normalize for scalar floats; the sign of a
                    // non-zero float is ±1, zero stays zero.
                    format!("(({0} > 0.0f) ? 1.0f : ({0} < 0.0f) ? -1.0f : 0.0f)", args_s[0])
                }
